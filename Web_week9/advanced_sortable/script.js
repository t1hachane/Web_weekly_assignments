...
function sortTbl(tbl) {
    ...
    for (var i = 1; i < tbl.rows.length; i++) {
        for (var j = i + 1; j < tbl.rows.length; j++) {
            if ((dir == "asc" && tbl.rows[i].cells[col].innerHTML.toLowerCase() > tbl.rows[j].cells[col].innerHTML.toLowerCase()) 
            || (dir == "desc" && tbl.rows[i].cells[col].innerHTML.toLowerCase() < tbl.rows[j].cells[col].innerHTML.toLowerCase())) {
